...
import { getSuperapps } from 'apiSdk/superapps';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['team-member', 'business-owner', 'delivery-partner', 'service-provider'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SuperappInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/superapps` : null),
    () => getSuperapps({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Superapps',
      path: isTenantUser ? `/superapps/view/${data?.[0]?.id}` : '/superapps',
      entity: 'superapp',
